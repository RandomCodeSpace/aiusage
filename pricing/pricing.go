...
// vendor, goose's provider-reported figure). This cost, stamped by the adapter,
// is the source of truth and is never overwritten by the price ladder — the
// ladder is a public-rate-card estimate of the same charge, and letting the
// estimate win is a strict loss of fidelity. Adapters that do not emit cost
// rely on pricing.Pricer to consult the ladder at collect time. An event left
// unpriced (Pricer returns ok=false or is nil) is stored with nil
// CostMicroUSD — the honest state, since a stored 0 would claim the request
// was free.
//
// LONG-CONTEXT TIERS are whole-request rate cards selected by prompt size, not
// marginal breakpoints. Every provider measures the boundary on input tokens
// (cached or not) and switches cards for the FULL request — OpenAI over 272K
// input, Google over 200K, etc. Charged at Rates.Long if the request's
// ContextTokens exceed the model's threshold, never marginal. The Charge type
// unifies per-request and aggregated pricing, and Aggregate marks the latter so
// a thousand short turns can never add up to one long-context request.
//
// EMBEDDED SNAPSHOT: the go:embed'ed filtered LiteLLM table is regenerated by
// internal/pricing/gensnapshot.go and is guaranteed to provide a baseline price
// for any model that appears in published documentation. It keeps a firewalled
// install pricing while offline. Partial config overrides can reach tables below
// the topmost rung (overrides fill missing fields from the rung they displace),
// and the snapshot is the floor: a model it cannot price remains unpriced.
//
