...
func TestSQLiteLiteralPathsRemainObservational(t *testing.T) {
	read := func(path string) (bool, error) {
		costs, err := loadSubAgentCosts(path)
		if err != nil {
			return false, err
		}
		if len(costs) != 1 || costs["sentinel"] != 123456 {
			return false, fmt.Errorf("wrong costs: %v", costs)
		}
		return true, nil
	}
	for label, name := range map[string]string{"question": "question?mode=rwc.db", "fragment": "fragment#.db", "percent": "percent%3F.db", "spaces": "with spaces.db", "unicode": "東京.db"} {
		for _, form := range []string{"absolute", "relative"} {
			t.Run(label+"/"+form, func(t *testing.T) {
				...
				if _, err := db.Exec(`CREATE TABLE assistant_usage_events(parent_tool_call_id TEXT,total_nano_aiu INTEGER); INSERT INTO assistant_usage_events VALUES('sentinel',123456)`); err != nil {
					db.Close()
					t.Fatal(err)
				}
