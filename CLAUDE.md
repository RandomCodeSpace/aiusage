...
UPDATE / BEFORE DELETE triggers in store/schema.sql abort any
...
in store reads two dimensions, or both this table and activity_events,
in one statement.
...
`pricing/gensnapshot.go` through `pricing.SnapshotField` — the
...
adapters and the store; `cmd` wires everything. Shared types belong in `model`,
not in a sideways import.

The adapter CONTRACT package imports no concrete adapter, and must not: a
consumer that only wants the interface should not link fifteen file-format
parsers, and package `adapter` importing the packages that import it is a cycle
besides. The fan-out is `adapter/all` (`Default()`), which `internal/cmd`
consumes like anyone else. Shared adapter plumbing hides one level down in
`adapter/internal/` — geminishape and tokenutil are helpers our own adapters
share, and promoting the tree they sat in would have made them API surface
nobody decided on.
...
store/query.go — as lexically sortable local wall-clock strings
...
(TestRollupFoldsSubHourEventsInKolkata in store).
...
in store/migrate.go: additive statements only (ALTER TABLE ... ADD
