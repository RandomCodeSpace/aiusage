...
`.go-version` pins CI and release to 1.26.5. `GOTOOLCHAIN=local` prevents
automatic upgrades, and the release hook checks the supported range before
building. `go.mod` retains the 1.25.13 minimum; dependencies are unchanged.

Hosted CI, native macOS execution, live-provider behavior, the full repository
suite, and the million-event CI performance contract were not run in this pass.
...
the baseline and changed configuration. Parsing with GoReleaser 2.18.0 remains
unverified; no release was attempted.
